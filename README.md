...
When a tag is pushed to this github repository, a new Docker image is automatically built and pushed to this package. The resulting docker image is tagged with the same git tag string (as returned by `git describe --tags --always`).
...
optional arguments:
  ...
  --slot {2,3,4,5,6,7}  Connect a GUI to pysmurf server running on a AMC
                        ...
                        used is "9000+3*slot_number". Ignored if option "--
                        port" is used.
...
- A `pysmurf server` application, running on an AMC carrier, uses by default port number `9000 + 3 * slot_number`,
